...
namespace Shakespeare.AST
{
    ...
    public class Shakespeare2C
    {
        public AstNodeCreator PlayNode
        {
            get
            {
                return (Irony.Ast.AstContext context, Irony.Parsing.ParseTreeNode treeNode) =>
                    {
                        var program = treeNode.ChildNodes[1].ToString();
    ...
    public class PlayNode : ShakespeareBaseAstNode
    {
        ...
        public override ShakespeareBaseAstNode OutputTo(TextWriter tw)
        {
            ...
            foreach (var ch in cdl.Characters)
                tw.WriteLine(ch.Declaration);
            
            tw.WriteLine("\tint comp1, comp2;");
            tw.WriteLine("\tglobal_initialize();");
            foreach (var ch in cdl.Characters)
                tw.WriteLine(ch.Initialization);
    ...
    public class EnterNode : ShakespeareBaseAstNode
    {
        ...
        public override ShakespeareBaseAstNode OutputTo(TextWriter sw)
        {
            ...
            {
                sw.WriteLine("\tenter_scene({0}, {1});", Location.Line, ch.AstNode.ToString().str2varname());
